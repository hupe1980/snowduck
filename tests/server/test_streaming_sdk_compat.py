...
def server_url() -> "Iterator[str]":
    ...
    server_proc = subprocess.Popen(
        [sys.executable, "-m", "uvicorn", "snowduck.server:app",
...
class TestRESTAPIContract:
    ...
    def test_open_channel_with_offset_token(self, test_client: Any) -> None:
        ...
        token_response = test_client.post(
            "/oauth/token",
            data={"grant_type": "urn:ietf:params:oauth:grant-type:jwt-bearer", "scope": "test"},
            headers={"Content-Type": "application/x-www-form-urlencoded"},
        )
        token = token_response.text
...
class TestRealSDKIntegration:
    ...
    def test_sdk_drop_channel(self, server_url: str) -> None:
        ...
        try:
            ...
            assert len(data.get("channels", [])) == 0, f"Channel should be dropped on server, got: {data}"
        finally:
            client.close()
